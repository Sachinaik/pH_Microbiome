---
title: "Fig7_RDA_Corelation_Shoot"
...
#Figure 7 Global RDA _Spearman corelation_abundance_Prevelance
#1) Read Data and Prepare Phyloseq
########################################
list.files()
# Replace with your actual metadata CSV file
sample_info_tab <- read.csv("SNGH4N_shoot.csv", 
                            header = TRUE, 
                            row.names = 1,
                            check.names = FALSE)
...
# Instead of merging, simply replace the sample data
sample_data(physeq_filterednochloromito) <- sample_data_object

# Continue with the rest of your code using physeq_filterednochloromito
...
# If DCA1 axis length, DCA1 < 3, use RDA; if DCA1 > 4, use CCA; if between 3-4, either is acceptable

```

```{r}
########################################
# Global RDA Analysis (Full Dataset)
########################################
...
# Step 7: Test significance of the global RDA model
...
# Step 11: Extract scores for plotting
sites <- scores(significant_vars, display = "sites")
species <- scores(significant_vars, display = "species")
...
# Rebuild RDA model
refined_rda_clean <- rda(genus_clean ~ pH + Plant + Fe + N, 
                         data = env_clean)
...
env_data <- env_data %>%
  ...
  mutate(Plant = factor(Plant, 
                        levels = c( "Geranium", "Marigold", "Petunia", "Tomato")))
...
env_data <- env_data %>%
  mutate(Plant = case_when(
    ...
    Plant == "petunia" ~ "Petunia",
    Plant == "tomato" ~ "Tomato",
    TRUE ~ Plant  # Keep any other values as-is
  )) %>%
  mutate(Plant = factor(Plant, 
                        levels = c( "Geranium", "Marigold", "Petunia", "Tomato")))
...
custom_colors2 <- c("#E69F00", "#56B4E9", "#009E73", "#d62728")  # Gradient colors
...
env_scores <- scores(significant_vars, display = "bp") %>%
  ...
  mutate(
    is_sig = Variable %in% c("pH", "Fe", "N"), # Significant variables
    RDA1_adj = RDA1 * 1.1, # Offset for asterisks
    RDA2_adj = RDA2 * 1.1,
    # CRITICAL ADDITION: Add color column - pH gets blue, others get red
...
global_plot <- ggplot() +
  ...
  # Start new color scale for arrow points
  new_scale_color() +
  ...
  # Labels and titles
  labs(title = "Global RDA of genus-level microbial community: Shoot tissue nutrients",
...
all_genus <- Genus_table[, all_percent, drop = FALSE]
```
...
########################################
# List of nutrients in your metadata (all nutirent with VIF >10 was chosen)
nutrients <- c( "Fe","N", "Al", "K" )
...
create_correlation_plot <- function(cor_matrix, p_matrix, title) {
  ...
  # Filter for both significant correlations (p < 0.05) AND |correlation| >= 0.6
...
# Extract just the data for all genera
genus_data <- as.data.frame(all_genus)
...
ggsave("RDA_plots/Figure7_combined_plotRDAshoot.jpeg", plot = combined_plot, width = 14, height = 16, dpi = 600)
ggsave("RDA_plots/Figure7_combined_plotRDAshoot.pdf", plot = combined_plot, width = 14, height = 16, dpi = 600)
ggsave("RDA_plots/Figure7_combined_plotRDAshoot.tiff", plot = combined_plot, width = 14, height = 16, dpi = 600)
